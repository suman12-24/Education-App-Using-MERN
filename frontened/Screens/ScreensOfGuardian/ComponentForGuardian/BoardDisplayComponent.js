...
const BoardDisplayComponent = () => {
    const windowWidth = Dimensions.get('window').width;
    const navigation = useNavigation();

    // Format the board name for display (convert API names to user-friendly display names)
    ...
    const renderBoardItem = ({ item }) => {
        ...
        const imageStyle = [
            styles.boardImage,
            // Apply custom dimensions only to the WBSE item
        ...
        <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
            <View style={styles.headerContainer}>
                <View style={styles.titleWrapper}>
                    <MaterialIcons name="school" size={23} color="#3366ff" />
                    <Text style={styles.sectionTitle}>
                        Secondary Educational Boards
                    </Text>
                </View>
                <TouchableOpacity style={styles.viewAllButton}
                    onPress={() => navigation.navigate('ViewAllForSecondaryEducation')}
                >
...
const styles = StyleSheet.create({
    ...
    flatListContent: {
        paddingVertical: 5,
        paddingHorizontal: 5, // Adjusted to ensure content starts from the edge
    },
    cardWrapper: {
        marginHorizontal: 6,
    },
    card: {
        width: Dimensions.get('window').width * 0.35, // Ensure this is small enough to fit multiple items
        borderRadius: 16,
        backgroundColor: '#fff',
...
export default BoardDisplayComponent;

// analyze the above code , and make a new page where all the boards will be displayed when i click on 'view all' button